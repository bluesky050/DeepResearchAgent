"""Run GitHub PR code review through AgentBus.

The bus submits the code review task to the planning agent, which coordinates
specialized review agents (quality, security, performance, test coverage) to
analyze the PR and generate a comprehensive review report.

Usage:
    python examples/run_code_review_bus.py --repo owner/repo --pr 42
    python examples/run_code_review_bus.py --repo owner/repo --pr 42 --max-rounds 10
    python examples/run_code_review_bus.py --config configs/code_review_bus.py --repo owner/repo --pr 42
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Run GitHub PR code review through AgentBus")
    parser.add_argument(
        "--config",
        default=os.path.join(root, "configs", "code_review_bus.py"),
...
async def main():
    ...
    # ------------------------------------------------------------------
    # 3. Build code review task
    # ------------------------------------------------------------------
    task_content = f"Review GitHub repository {args.repo} PR #{args.pr}. Perform comprehensive code review including quality, security, performance, and test coverage analysis. Generate a detailed review report and post it to the PR."

    ctx = SessionContext()
    task = Task(content=task_content, session_id=ctx.id)

    logger.info("=" * 60)
    logger.info(f"| Code Review Task")
    ...
    # ------------------------------------------------------------------
    logger.info("| Submitting code review task to AgentBus...")
    ...
    if success:
        ...
        logger.info(f"| Code review failed")
    ...
    # 6. Shutdown
    # ------------------------------------------------------------------
    logger.info("| Shutting down AgentBus...")
    await bus.shutdown()
    logger.info("| Code review session complete")
